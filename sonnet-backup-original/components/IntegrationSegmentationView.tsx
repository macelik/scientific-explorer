  ...
  const trackFig = useMemo(() => {
    ...
    srcs.forEach((src, k) => {
      ...
      if (seg.labels.small) for (const s of smallHere.filter((s) => s.source === src)) {
        ...
        const color = seg.scale === 'binned' ? binColor(mv.value, metricDef.bins) : (mv.value === null ? '#e5e7eb' : viridisAt(Math.min(1, Math.abs(mv.value) / (metricDef.kind === 'stat' ? 8 : 2))))
        const selected = seg.selectedSegment === s.key
        const dash = cc.cls === 'weak' ? 'dot' : cc.cls === 'inconsistent' ? 'dash' : 'solid'
        const x0 = mb[s.s], x1 = s.e < chromMeta.n ? mb[s.e] : bins[2 * (chromMeta.offset + chromMeta.n - 1) + 1] / MB
        shapes.push({ type: 'rect', xref: 'x', yref: `${yref} domain`, x0, x1, y0: 0, y1: 1, fillcolor: color, opacity: selected ? 0.75 : 0.5, line: { width: selected ? 2.5 : (seg.labels.consistency ? 1.2 : 0.4), color: selected ? '#111827' : '#111827', dash: seg.labels.consistency ? dash : 'solid' }, layer: 'below' })
        const labels: string[] = []
