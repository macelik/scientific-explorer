...
function ExperimentResults({ result, outdated, onRegion }: { result: ExperimentResult; outdated: boolean; onRegion?: (start:number,end:number)=>void }) {
  ...
  return <section className="experiment-results" aria-label="Experiment results">
    ...
    <p className="muted">Drag horizontally to select a new source region. Dashed markers show each scenario’s root candidate. All curves use unsmoothed X; coordinate gaps break the lines.</p>
    <details className="scientific-details"><summary>Both children and exact split tests</summary>
      <p>Each child is recomputed on its own interval. Bounds can differ between scenarios. Hypothetical children are inspected even when the parent fails; they do not enter automatic segmentation.</p>
      ...
      <div className="table-scroll"><table className="tbl"><thead><tr><th>Scenario / node</th><th>Bins [start, end)</th><th>Candidate</th><th>AD</th><th>Local p</th><th>Global p</th><th>Automatic decision</th></tr></thead><tbody>
        {([['Original', result.original], ['Edited', result.edited]] as const).flatMap(([label, tree]) => tree.nodes.map(node => <tr key={label+node.side}>
          <td>{label} / {node.side}</td><td>[{node.start}, {node.end})</td><td>{bp(node.argmax_b === null ? null : node.start+node.argmax_b)}</td><td>{number(node.argmax_ad)}</td>
...
export default function ExperimentView() {
  ...
  useEffect(() => {
    ...
    initialize(key, { start: idx.length ? idx[0] : 0, end: idx.length ? idx[idx.length-1]+1 : Math.min(40, cm.n), operation: 'multiply', value: 0.5, preserve_total: false, cn_state:'loss',direction:'right',length:40,seed:42 })
  }, [key, cm, root])
