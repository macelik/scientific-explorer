...
export default function Karyogram({ rows, ruleMatch, onOpen }: Props) {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      if (el.clientWidth > 100 && Math.abs(el.clientWidth - last) > 2) { last = el.clientWidth; setSize({ w: el.clientWidth, h: Math.min(720, Math.max(320, Math.round(el.clientWidth * 0.5))) }) }
    })
